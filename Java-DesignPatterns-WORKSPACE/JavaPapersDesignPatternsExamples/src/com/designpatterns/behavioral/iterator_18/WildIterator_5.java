...
public class WildIterator_5 implements Iterator_4 {
	public List<Animal_1> animalList;
	private int position;

	public WildIterator_5(List<Animal_1> animalList) {
	...
	public Animal_1 nextAnimal() {
		...
		for (; position < animalList.size(); position++) {
			if ("Wild".equals(((Animal_1) animalList.get(position)).getAnimalType())) {
	...
	public boolean isLastAnimal() {
		for (int i = position; i < animalList.size(); i++) {
			if ("Wild".equals(((Animal_1) animalList.get(i)).getAnimalType())) {
				return false;
			}
		}
		return true;
